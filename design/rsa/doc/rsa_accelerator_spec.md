...
**版本**：v0.1（所有 18 项决策已完成）  
**最后更新**：2026-04-20  
**场景定位**：安全加速卡（原型阶段聚焦简单电路）
...
| 13 | 功耗预算 | ✅ 已确认 | **不设置**（与决策 #7"仅 RTL 仿真"一致，无物理实现） |
| A | 私钥模幂算法 | ✅ 已确认 | Montgomery Ladder + CRT（原型版不启用 blinding / fault check，见 4.3） |
...
**安全加速卡**：以独立硬件卡形式插入服务器，通过 APB 32-bit 主机接口卸载 CPU 的
非对称加密运算。
...
- **简单优先**：三层嵌套 FSM（顶层 / Ladder / MontMul），不做微码与指令集
- **功能优先**：先跑通签名/验签；性能、安全、功耗指标均放到未来版本
- **可扩展性后置**：不为将来 ECC/PQC 预留通用接口
- **仅 RTL 仿真**：不做综合、不绑定工艺、不设置功耗预算
...
本版本**不设置功耗预算**，也不引入 RTL 级功耗优化手段（如细粒度 clock gating、
operand isolation）。

理由：
- 决策 #7 已将综合/工艺/物理实现排除在范围外；无物理实现则无法度量也无意义优化
- 保持 spec 与实际能力一致，避免写出无法验证的"纸面指标"
- clock gating、电源域、arg isolation 等功耗优化属于物理实现阶段事项，
  在后续产品化工程（新建）中统一处理
...
详细方案待 RTL 基本成形后细化。整体思路：

- **框架复用**：参考 `verif/sjtag2apb`（UVM）与 `verif/sjtag2apb_cocotb`（cocotb）
  的 APB 验证资产
- **黄金模型**：Python 直接使用 `pow(m, d, n)` 作为参考值（或 OpenSSL / GMP 对拍）
- **典型测试集**：
  - 已知向量：NIST FIPS 186 RSA 向量
  - 随机测试：随机 `(p, q, e, m)` 组合多轮对比
  - 边界值：`m=0 / m=1 / m=n-1` 等
  - CRT 支路：分别验证 `mp`、`mq` 中间结果
  - 公钥快速路径：`e=65537` 下 Verify/Encrypt 对拍
- **覆盖率**：FSM 状态、CIOS 两层循环边界、Ladder bit 分支
...
| 维度 | 结论 |
|------|------|
| 产品定位 | 安全加速卡（原型阶段聚焦简单电路） |
| 算法范围 | 仅 RSA |
| Key 长度 | 仅 RSA-2048 |
| 操作 | Sign / Verify / Encrypt / Decrypt 全支持，两条硬件路径 |
| Padding | 仅 Raw 模幂，padding/Hash/RNG 全由软件 |
| 私钥模幂 | Montgomery Ladder + CRT（无 blinding / fault check） |
| 公钥模幂 | 复用 Montgomery Ladder，循环 17 次（e=65537） |
| 模乘算法 | Montgomery Multiplication — CIOS |
| 数据通路 | 32-bit MAC，2048b 分 64 limb |
| 控制方式 | 三层嵌套纯 FSM |
| 主机接口 | APB 32-bit |
| 密钥槽 | 0（Key Vault 不实现） |
| FIPS 等级 | 不声明 |
| TRNG | 不内置，顶层预留 `rand_in/valid/req` 接口 |
| Hash 引擎 | 不内置，不预留接口 |
| 工艺 / 频率 / 功耗 | 仅 RTL 仿真，标称 100 MHz，不设功耗预算 |
| 性能目标 | ≥ 20 RSA-2048 Sign ops/s @ 100 MHz |
